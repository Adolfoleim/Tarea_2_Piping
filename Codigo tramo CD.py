...
from fluids.fittings import exit_normal
...
# Filtrar tramo CD
df_CD = df[df["tramo"] == "C-D"].sort_values("punto_idx").reset_index(drop=True)

# Datos enunciado
Q_CD = 25 / 3600
n = 0.010
D = df_CD.iloc[0]["diametro_m_excel"]
...
for i in range(len(df_CD) - 1):
    yi = df_CD.iloc[i]["y_cm"] / 100
    yi1 = df_CD.iloc[i+1]["y_cm"] / 100
    Li = df_CD.iloc[i+1]["dist_seg_cm"] / 100
    dist_acum += Li
    S0 = (yi - yi1) / Li
...
ax1.set_xlabel("Distancia desde C [m]")
...
fig.suptitle("Perfil de velocidad y tirante – Tramo CD")
fig.tight_layout()
plt.show()
...
# Pérdidas por singularidades (solo entrada a D)
...
for i in range(len(velocidades)):
    if np.isnan(velocidades[i]):
        ...
        energia_total.append(energia_potencial[i]+ energia_cinetica[i]+ perdidas_friccion[i]+ perdidas_sing[i])
...
plt.xlabel("Distancia desde C [m]")
plt.ylabel("Energía [m]")
plt.title("Balance de energía – Tramo CD")
plt.grid(True)
...
ruta_salida = directorio_script / "Resultados_Tramo_CD.xlsx"
